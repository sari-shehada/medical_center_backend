...
from api.serializers import AddPatientSerializer,  PatientDisplaySerializer, DiagnosisDetailsSerializer, PatientMedicalCaseDetailsSerializer
...
def getById(request, userId):
    user = Patient.objects.filter(id=userId).first()
    if not user:
...
def getMedicalCases(request, userId):
    ...
    medical_cases = MedicalCase.objects.filter(diagnosisId__in=diagnosticsIds)
    return Response(PatientMedicalCaseDetailsSerializer(medical_cases, many=True).data)
